    ...
    <div class="md-layout-item md-medium-size-100 md-xsmall-size-100 md-size-100">
      <md-card>
        <md-card-header data-background-color="blue">
          <v-expansion-panels style="background-color: transparent;" accordion flat>
            <v-expansion-panel style="background-color: transparent;">
              <v-expansion-panel-header></v-expansion-panel-header>
              <v-expansion-panel-content style="background-color: white;">
                <v-row justify="center">
                  <v-col cols="sm-4 xs-12">
                    <v-select v-model="myLineChartLimit" v-on:change="changeLineChartLimit" :items="dropLimit" label="Datapoints" dense outlined></v-select>
                  </v-col>
                  <v-col cols="sm-4 xs-12">
                    <v-select v-model="myLineChartPeriod" v-on:change="changeLineChartPeriod" :items="dropTyp" label="Accumulation Period" dense outlined></v-select>
                  </v-col>
                </v-row>
          ...
          <p class="category">Balance Information</p>
        </md-card-header>
        <md-card-content>
          <v-tabs v-model="tabs">
            ...
            <v-tab-item id="Balance">
              <v-row dense justify="center">
                <v-col cols="sm-12 xs-12">
                  <v-card flat>
                    <v-card-text>
                      <line-chart v-if="options.loaded" :chart-data="linedatacollection" :options="lineoptions"></line-chart>
                    </v-card-text>
                  </v-card>
                </v-col>
              </v-row>
  ...
  export default {
    ...
    methods: {
      ...
      changeLineChartPeriod() {
        ...
        this.$store.commit("dashboard/setDashboardPerformanceUpdate", true);
        this.$store.commit("dashboard/setDashboardWinlossUpdate", true);
        this.getData();
      },
